  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="w-full md:w-1/2 relative h-[250px] sm:h-[350px] md:h-auto shrink-0 overflow-hidden bg-zinc-900/80">
          <div className="absolute inset-0 w-full h-full p-3 sm:p-4 md:p-6 flex items-center justify-center">
            <img 
              src={item.image} 
              alt={item.name} 
...
export default function App() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('https://formsubmit.co/ajax/sidreriaelembarcadero@hotmail.com', {
        method: 'POST',
        headers: {
              ...
              {filteredItems.map((item) => (
                ...
                >
                  <div className="glass-card rounded-[2rem] overflow-hidden border border-white/5 group-hover:border-brand-primary/50 group-hover:shadow-[0_0_30px_rgba(245,158,11,0.15)] group-hover:-translate-y-1 transition-all duration-300 flex flex-col h-full cursor-pointer" onClick={() => setSelectedItem(item)}>
                    <div className="relative aspect-[4/5] overflow-hidden shrink-0 bg-zinc-900/80">
                      ...
                      {item.badges?.map((badge: string) => (
                        <div key={badge} className="absolute top-5 left-5 bg-brand-primary text-brand-dark font-display text-sm px-4 py-1 uppercase shadow-xl transform -rotate-2">
                          {badge}
                        </div>
                      ))}
                      ...
                      <div className="absolute bottom-4 left-4 right-4 flex flex-wrap gap-2">
                        {item.tags.map(tag => (
                          <span key={tag} className="text-[10px] uppercase font-bold tracking-widest px-3 py-1 bg-black/55 backdrop-blur text-white rounded-full border border-white/10">
                            {tag}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="p-6 sm:p-7 flex flex-col flex-grow">
                      <div className="mb-4">
                        <h3 className="font-display text-2xl sm:text-3xl uppercase leading-tight">{item.name}</h3>
                      </div>
                      ...
                      <button 
                        ...
                        className="flex items-center gap-2 text-brand-primary text-xs uppercase tracking-[0.2em] font-bold group-hover:gap-4 transition-all self-start"
                      >
